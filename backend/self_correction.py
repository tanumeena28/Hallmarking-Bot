...
def auto_train_feedback(query_log_id: int):
    """
    ...
    try:
        ...
        print(f"Error during self-correction for log ID {query_log_id}: {e}")
    finally:
        db.close()

def trigger_self_correction(query_log_id: int):
    """Triggers the self correction in a background thread"""
    thread = threading.Thread(target=auto_train_feedback, args=(query_log_id,))
    thread.start()
